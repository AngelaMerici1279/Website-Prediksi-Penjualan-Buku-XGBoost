...
#     # --- TAMBAHAN REVISI: Load MAE Bulanan ---
#     if os.path.exists("mae_bulanan.pkl"):
#         mae_bulanan_ref = joblib.load("mae_bulanan.pkl")
#     else:
#         # Default jika file belum ada (menggunakan MAE global)
#         mae_bulanan_ref = {i: 59.7 for i in range(1, 13)}
...
#             # --- TAMBAHAN REVISI: Ambil MAE Spesifik untuk bulan m ---
#             mae_spesifik = mae_bulanan_ref.get(m, 59.7)
...
#            # Masukkan data ke forecast_json
...
#                 # --- DATA BARU UNTUK JAWAB REVISI ---
#                 'mae_spesifik': round(mae_spesifik, 2)
#             })
...
try:
    ...
    # --- REVISI: Load 3-Tier MAE ---
    mae_tier1 = {}
    mae_tier2 = {}
    mae_tier3 = {i: 59.7 for i in range(1, 13)}  # Default
    
    if os.path.exists("mae_tier1_full.pkl"):
        mae_tier1 = joblib.load("mae_tier1_full.pkl")
        print(f"✅ Tier 1 (Kombinasi lengkap): {len(mae_tier1)} entries")
    
    if os.path.exists("mae_tier2_kategori.pkl"):
        mae_tier2 = joblib.load("mae_tier2_kategori.pkl")
        print(f"✅ Tier 2 (Kategori+Bulan): {len(mae_tier2)} entries")
    
    if os.path.exists("mae_tier3_global.pkl"):
        mae_tier3 = joblib.load("mae_tier3_global.pkl")
        print(f"✅ Tier 3 (Global per bulan): {len(mae_tier3)} entries")
...
def predict():
    ...
    try:
        ...
        for i in range(1, 13):
            ...
            # --- STRATEGI 3-TIER MAE ---
            # Tier 1: Coba MAE kombinasi lengkap (paling spesifik)
            mae_key_full = (kat_id, prop_id, gudang_id, m)
            
            if mae_key_full in mae_tier1:
                mae_spesifik = mae_tier1[mae_key_full]
                tier_used = 1
            else:
                # Tier 2: Fallback ke MAE kategori + bulan
                mae_key_kat = (kat_id, m)
                if mae_key_kat in mae_tier2:
                    mae_spesifik = mae_tier2[mae_key_kat]
                    tier_used = 2
                else:
                    # Tier 3: Fallback ke MAE global bulan
                    mae_spesifik = mae_tier3.get(m, 59.7)
                    tier_used = 3
           ...
           # Masukkan data ke forecast_json
            forecast_json.append({
                ...
                'total_history': int(seasonal_sums.get(m, 0)),
                'rata_rata': int(round(seasonal_means.get(m, 0))),
                # --- DATA MAE SPESIFIK untuk kombinasi ini ---
                'mae_spesifik': round(mae_spesifik, 2)
            })
